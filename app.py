...
st.title("CyberGuard")
st.write("Cybersecurity risk assessment based on your everyday security habits.")
...
with st.form(f"risk_form_{v}"):
    ...
    submitted = st.form_submit_button("Check My Risk Score")
...
if st.session_state.prediction_made:
    ...
    <div class="status-card" style="border-left-color:{color};">
        <div class="status-title" style="color:{color};">{predicted_label} Risk</div>
    ...
    chart = alt.Chart(proba_df).mark_bar().encode(
        x=alt.X("Risk Level:N", sort=order, axis=alt.Axis(labelAngle=0)),
...
if st.session_state.prediction_made:
    ...
    st.caption("Ask why you got this risk score, or how to improve it.")
